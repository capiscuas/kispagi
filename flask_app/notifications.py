...
def new_notif(event, action_msg):
    ...
    n_title = '{} {} in project {}. Comment: {}'.format(username, action_msg, area_name, event_comments)
    n_content = 'Issue: {}'.format(task_title)
...
def check_time_event_changes(new, old):
    ...
    for id, event in new_events_dict.items():
        ...
        if id in old_events_dict:
            ...
            if total_hours_spent != old_total_hours_spent or voluntary_adj != old_voluntary_adj:
                notifications.append(new_notif(event, 'changed from {} {} hours to {} {} hours'.format(old_total_hours_spent, old_voluntary_adj,
                                                                                                       total_hours_spent, voluntary_adj)))
        else:
            notifications.append(new_notif(event, 'added {} {} hours'.format(total_hours_spent, voluntary_adj)))
    ...
    for id, event in old_events_dict.items():
        ...
        if id not in new_events_dict:
            notifications.append(new_notif(event, 'removed its {} {} hours'.format(total_hours_spent, voluntary_adj)))
